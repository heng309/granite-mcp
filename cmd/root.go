...
func loadSystemPrompt(filePath string) (string, error) {
	...
	// os.ReadFile reads the entire file specified by filePath and returns its contents as a byte slice.
	data, err := os.ReadFile(filePath)
	if err != nil {
		return "", fmt.Errorf("error reading system prompt file '%s': %w", filePath, err)
	}

	// Convert the byte slice containing the file data directly to a string.
